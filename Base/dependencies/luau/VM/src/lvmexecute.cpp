...
#define VM_REG(i) (LUAU_ASSERT(L && base && unsigned(i) < unsigned(L->top - base)), &base[i])
#define VM_KV(i) (LUAU_ASSERT(cl && cl->l.p && unsigned(i) < unsigned(cl->l.p->sizek)), &k[i])
#define VM_UV(i) (LUAU_ASSERT(cl && unsigned(i) < unsigned(cl->nupvalues)), &cl->l.uprefs[i])
...
void luau_execute(lua_State* L)
{
    ...
    __except (L && L->global->errorjmp ? (luaD_throw(L, LUA_ERRRUN), EXCEPTION_EXECUTE_HANDLER) : EXCEPTION_CONTINUE_SEARCH)
    {
    }
}
